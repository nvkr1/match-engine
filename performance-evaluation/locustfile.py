...
pair = os.getenv('PAIR') or 'BTC-USDT'
jtl_url = os.getenv('JTL_URL') or 'http://127.0.0.1'
jtl_token = os.getenv('JTL_TOKEN') or 'at-1abdf941-4307-4b5b-a113-76b7f8a17f77'
jtl_project = os.getenv('PROJECT') or 'Demo Exchange'
jtl_scenario = os.getenv('JTL_SCENARIO') or 'BTC-USDT'
jtl_environment = os.getenv('JTL_ENVIRONMENT') or 'test'
engine_url = os.getenv('ENGINE_URL') or f'http://localhost:5001/{pair}'
...
class JtlListener:
    ...
    def _upload_file(self):
        files = dict(
            ...
            status=(None, 1))
        url = '%s:5010/api/projects/%s/scenarios/%s/items' % (
...
orders = order_generator(Decimal(28000), Decimal(30000), Decimal(1)/1000, Decimal(1))
...
class MyUser(FastHttpUser):
    ...
    def index(self):
        ...
        resp = self.client.post("/order/limit", json=body)
        cancel_chance = random.randrange(100) <= 10
        if cancel_chance:
